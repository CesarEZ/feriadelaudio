...
class Admin_roles extends MY_Controller
{
	...
	public function module_add(){
		...
		if($this->input->post('submit')){
			...
			if ($this->form_validation->run() == FALSE) {
				...
				if($result){
					$this->session->set_flashdata('success', 'El módulo ha sido agregado exitosamente!');
	...
	public function module_edit($id=0){
		...
		if($this->input->post('submit')){
			...
			if ($this->form_validation->run() == FALSE) {
				...
				if($result){
					$this->session->set_flashdata('success', 'El módulo ha sido actualizado exitosamente!');
					redirect(base_url('admin/admin_roles/module'));
				}
			}
		}
	...
	public function module_delete($id=''){
		...
		$this->session->set_flashdata('msg','El rol ha sido eliminado exitosamente.');	
	...
	public function sub_module_add(){
		...
		if($this->input->post('submit')){
			...
			if ($this->form_validation->run() == FALSE) {
				...
				if($result){
					$this->session->set_flashdata('success', 'El submódulo ha sido agregado exitosamente!');
	...
	public function sub_module_edit($id = 0){
		...
		if($this->input->post('submit')){
			...
			if ($this->form_validation->run() == FALSE) {
				...
				if($result){
					$this->session->set_flashdata('success', 'El submódulo se ha actualizado correctamente!');
					redirect(base_url('admin/admin_roles/sub_module/'.$parent));
				}
			}
		}
	...
	public function add(){
		...
		if($this->input->post('submit'))
		{
			...
			$this->session->set_flashdata('success', 'Registro agregado exitosamente');	
			redirect('admin/admin_roles');
		}
