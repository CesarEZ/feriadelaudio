...
class Setting extends Main_Controller {
	...
	{	
		if ($this->input->post('submit')) {
			...
			$this->form_validation->set_rules('old_password','Contraseña anterior','trim|required|min_length[3]');
			$this->form_validation->set_rules('new_password','Nueva contraseña','trim|required|min_length[3]');
			$this->form_validation->set_rules('confirm_password','Confirmar contraseña','trim|required|min_length[3]|matches[new_password]');
			...
			if ($this->form_validation->run() == FALSE) {
				...
				if($result) {
					$this->session->set_flashdata('update_password_success','La contraseña del usuario se ha actualizado correctamente');
					
					redirect(base_url('setting/change_password'));
				}else{
					$this->session->set_flashdata('update_password_failed','Antigua contraseña es incorrecta');
					redirect(base_url('setting/change_password'));
				}
			}
		}
